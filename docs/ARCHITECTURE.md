...
**Current milestone:** M5.1 — *M5.0's exact area transport + GPU-side exact-reject fast paths (per-piece AABB bookkeeping; blockers provably disjoint from every surviving piece skip hull and carve; no-carve pixels return kRect directly) + the L6 rect restriction (blocker hulls S-H-clipped to the emitter rect before the carve — every piece is a subset of the rect, so piece ∩ B = piece ∩ (B ∩ E) and restricting B is region-exact; only edges that could never cut a piece disappear) + the L7 sphere frustum reject (when ymin ≤ 1e-4 the L2 disk bound can never fire — the floor-sphere regime — but a side half-space of the rect pyramid conv(P, E) separating the sphere strictly beyond the plane by > R proves the tangent cone cannot reach the rect cone, so the 33 samples cannot carve; sqrt-free, fires pre-sample) + the L8 ring-trig constant table (the 32 ring angles 2πk/32 fold to kRingCS constants; only the conic-vertex angle pays runtime trig). Every skip is one-sided, so the union is unchanged — the equivalence contract is fuzz-verified in check_area_model.py section 8 (per-fired soundness — L7's re-runs the FULL sampled pipeline and asserts its restricted hull is empty — UNCAPPED ref-vs-skips equivalence to 1e-9, the shipped cap scored against the exact baseline net-better: 2.996e-1 → 5.675e-3 on the L7-extended generator; section 9 measures the L8 θ-literal impact at 3.9e-14). The 0.5.1-draft inclusion-exclusion rewrite was rejected on hardware (worse image AND fps) and reverted.* (M5.0: the true area light: the Cornell emitter is
now the frozen 1.30 × 1.05 m patch (exact visible-patch form factor under
analytic backprojection visibility); the 16-point grid retires to the
`--area-light 0` fallback with every M4.x replay pin.* (This header was
stale at M3.3 through M4.0.9 — the CHANGELOG and
benchmarks/cornell_box/BASELINE.md are the authoritative history; keep this
line current from now on.)
...
| Analytic lateral half-plane in the centroid shadow path (M4.0.9.1): the occluder set rides along as uniforms (≤ 4 boxes + ≤ 4 spheres, built from the occluder meshes' world AABBs — the same meshes the heightfield rasterizes), and each march sample grades `g_lat = clamp(0.5 + r/(E_perp·min(t,tCap)), 0, 1)` with r the signed 2D ground distance to the nearest eligible primitive, min-combined with the vertical half-plane grade | The M4.0.9 centroid grade was exact in-plane but structurally blind to the LATERAL penumbra — receivers whose centroid ray skims past a footprint never touch a real column, so the lateral silhouette rendered as a full cliff (the hardware row's +1.2 edge RMSE; the field verdict "very sharp edges"; no penumbra width fixes a cliff). The analytic set costs zero new texture taps (pure ALU per sample), stays continuous at every penumbra width, and cannot disagree with the captured field because both derive from the same meshes. The soft early-out gained the correctness-driven B(t) ≥ 0 gate (with B < 0 the lateral grade can still undercut the vertical bound). `--shadow-lateral 0` reproduces the M4.0.9 centroid march bit-for-bit. | Superseded by backprojection (project the blocker silhouette through the receiver, clip against the emitter rect — exact for convex occluders) when M5's real `AreaLight` objects land; the half-plane ramps are its per-axis 1D reductions. |
...
| Forward renderer, lights enumerated per fragment (uniform loop, kMaxPointLights = 16) | A handful of lights and objects; simplest correct thing. The renderer still only knows "draw this mesh" — scene/material abstraction (M5) is the prerequisite for renderer-owned decisions. | Area-light transport (M5.0): the emitter is a first-class `AreaLight` (engine/src/rendering/AreaLight.h, header-only struct + CPU reference transport); the PBR shader's `uAreaOn` branch evaluates the DIFFUSE as the Arvo form factor of the VISIBLE patch polygon (emitter rect minus each occluder's backprojected region, first-outside-edge piece decomposition — area- and Arvo-exact for any simple subject, bridges cancel in the edge sum) and the SPECULAR as a representative-point GGX with the VOS solid angle. Boxes backproject their two height-cap footprints (the exact radial sweep); spheres backproject their tangent cone sampled by 33 direction-space directions with far-clamped sub-horizontal generators. Validated end-to-end against float64 brute force before any GLSL (scripts/check_area_model.py: form factor 1e-9, boxes/unions exact to the grid pitch, sphere ≤ 0.9% of K one-sided); the naive per-blocker product it replaced over-darkened a cbox03 overlap band by 18.4% | The 16-point grid was a documented approximation of the area emitter (its 16-superposition penumbra, its point falloff, its quantized transport); M4.x's window/centroid/lateral work progressively de-quantized its artifacts without removing the root cause. Backprojection is the exactness class the heightfield already had (convex + static) with zero texture taps, zero march, zero jitter, C1-continuous penumbrae, contact hardening, and emitter-shape anisotropy for free. `--area-light 0` keeps the grid transport byte-identical — every M4.x replay pin lives under it | The grid stays as the `--area-light 0` fallback and for the demo scene; documented residuals (sphere inscribed-conic under-block, representative-point specular) are queued behind ledger evidence; the scene/material abstraction (scene.json loader, MeshInstance) remains the M5.2 follow-up |
| Clustered light assignment when light counts grow (research track); renderer-known scene graph at M5. |
