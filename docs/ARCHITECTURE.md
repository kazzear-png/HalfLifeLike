...
**Current milestone:** M5.0.1 — *M5.0's exact area transport + GPU-side exact-reject fast paths (per-piece AABB bookkeeping; blockers provably disjoint from every surviving piece skip hull and carve; no-carve pixels return kRect directly). Every skip is one-sided, so the union is unchanged — the equivalence contract is fuzz-verified in check_area_model.py section 8 (SAT soundness, no-region-loss direction, brute adjudication). The 0.5.1-draft inclusion-exclusion rewrite was rejected on hardware (worse image AND fps) and reverted.* (M5.0: the true area light: the Cornell emitter is
