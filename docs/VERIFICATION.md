...
- [ ] `ctest --test-dir build --output-on-failure` runs all four suites and passes:
      ...
      - `check_area_model` — **9 sections, ALL OK** (M5.1 gate: the float64 model battery + the exact-reject equivalence fuzz — 4000 iterations of jittered rigs (floor + elevated + FLOATING spheres, the L7 clamp stress case) asserting per-fired-reject soundness — L7's being the strongest: each fired frustum reject re-runs the FULL sampled pipeline and asserts its restricted hull is EMPTY — plus UNCAPPED ref-vs-skips equivalence to 1e-9 relative (the L1–L7 exactness proof: both sides tile the same region), the shipped 16-piece cap scored against the exact uncapped baseline under a net-better gate (total cap error ref 2.996e-1 vs skips 5.675e-3, ~52×, on the L7-extended generator), and frozen-config agreement to 1e-12 relative; section 9 = the L8 receipt, the pre-L8 θ-literal's whole-pipeline impact measured at 3.9e-14 relative, gate 1e-12)
